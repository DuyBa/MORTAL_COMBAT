...
start_button = button.Button(180, 220, start_img, 0.8)
quit_button = button.Button(582, 220, quit_img, 0.8)
...
# 		if menu_state=="hero":
# 			screen.fill((52, 78, 91))
# 			draw_text("SELECT HERO1", font, WHITE,screen, 40, 50)
...
# 			draw_text("SELECT HERO2", font, WHITE,screen, 40, 300)
...
while run_of_main[0]:
	...
	#check if the options menu is open
		...
		dau_vao.play_immediately(totally_quit, run_of_pause, run_of_fighting, run_of_main, run_of_end)
	for event in pygame.event.get():
		if event.type == pygame.QUIT:
			run = False
